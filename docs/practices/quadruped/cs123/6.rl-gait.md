...
sidebar_position: 6
title: 6. 策略训练
description: "用 PPO 训练 Pupper 跟随速度命令：Gymnasium 环境、45/50 维观测、PD 残差动作、奖励设计和真实训练结果。"
...
当前观测没有直接提供机身线速度。策略需要从关节状态、姿态变化以及多步交互中间接推断速度，这也是低速跟踪仍会超调的原因之一。
...
速度与稳定性得到保留，但 0.485 的接触匹配接近未按相位区分触地的基准水平。当前实验**没有成功形成标准 walk、trot 和 pace**，只能证明 gait 输入、相位、奖励、迁移和评估链路已经跑通。
...
当前版本的定位是“便于读懂、能够训练和有真实报告的最小实验”。主要局限包括：

1. 观测没有机身线速度，也没有历史帧堆叠；
2. 没有观测噪声、动作延迟、外力扰动或模型参数随机化；
3. 奖励没有单独处理静止、垂直速度、足端打滑、膝盖/身体碰撞等情况；
4. 足端接触使用 site 高度近似，不能覆盖所有真实接触情况；
5. gait 迁移尚未形成可靠的目标接触时序；
6. 只在平地仿真中验证，没有进行真机部署。

合理的下一步是：先加入低速/静止专门约束和机身线速度观测，再做 gait 从零公平对照；基础策略稳定后，再逐步加入观测噪声、延迟、摩擦/质量随机化和外力扰动。每次只增加一类机制，并用相同 checkpoint 评估协议记录变化。
