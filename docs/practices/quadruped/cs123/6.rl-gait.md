...
![指数速度奖励有上界：误差越小越接近 1，误差增大时平滑衰减。](./figs/exp-velocity-reward.webp)
...
![20M 步基础训练曲线：回报和回合长度总体上升，但回合长度尚未稳定达到 1000；策略标准差持续增大。](../../../../codes/practices/quadruped/cs123/6.rl_pupper/reports/training_curves.webp)

下图展示最佳 19M checkpoint 对 `0.2/0.4/0.6 m/s` 三档前向速度命令的响应。中高速命令已经能被跟踪，低速段的波动和超调仍然明显。

![最佳 19M checkpoint 的速度跟踪结果：红色虚线为目标速度，蓝线为机身坐标系下的实际前向速度。](../../../../codes/practices/quadruped/cs123/6.rl_pupper/reports/velocity_tracking.webp)

最终策略的命令切换效果如下。GIF 依次包含站立、前进、左转、后退和再次站立；它可以完成主要动作，但切换过程尚未完全稳定。

![19M checkpoint 的命令切换演示。](../../../../codes/practices/quadruped/cs123/6.rl_pupper/reports/command_demo.gif)
...
![最佳 4.5M gait checkpoint 的目标接触时序与实测足端接触。](../../../../codes/practices/quadruped/cs123/6.rl_pupper/reports/gait_finetune/contact_patterns.webp)

![依次输入 walk、trot、pace 的 gait 演示；三段运动的视觉和接触差异仍不充分。](../../../../codes/practices/quadruped/cs123/6.rl_pupper/reports/gait_finetune/gait_demo.gif)
...
![最终版 0.15 m/s 巡航演示：与第 5 章手写 trot 同速档，闭环平衡下接近其节律与观感。](../../../../codes/practices/quadruped/cs123/6.rl_pupper_v2_5/reports/cruise_demo.gif)

![最终版标准命令序列演示：站立、前进 0.5 m/s、左转、后退、站立，全程零摔倒。](../../../../codes/practices/quadruped/cs123/6.rl_pupper_v2_5/reports/command_demo.gif)
