...
![执行器工作流程：输入信号 → 能量转换 → 产生动作 → 反馈回路](./figs/actuator-workflow.webp)
...
![常见电动执行器示意图](./figs/electric-actuators.webp)
...
![齿轮传动动画：主动轮与从动轮按固定传动比反向旋转，输出端转得更慢但更有力](./figs/gear-ratio-demo.gif)
...
**开环控制(Open-loop Control)** 的控制量只依赖目标 $q_d$ 和时间 $t$，不读取系统当前状态 $q$，如下公式所示：
...
![开环控制：指令单向下发，执行端状态不回传](./figs/open-loop-control.webp)

开环控制的优点是**简单**，不需要传感器建立反馈通路。但缺点是**无法适应任何外界扰动**，例如机械臂收到"转到 90°"的开环指令，一旦碰到摩擦、负载或障碍物，就会停在偏离 90° 的位置，并且不会自我修正。

### 1.2.2 闭环控制

**闭环控制(Closed-loop Control)** 又称**反馈控制(Feedback Control)**，控制量根据实测误差 $e = q_d - q$ 实时计算，如下公式所示：
...
![闭环控制：传感器把实测状态回传，控制器据此修正](./figs/closed-loop-control.webp)
...
**Bang-Bang 控制**（又称双位控制或开关控制）是闭环控制里最简单的一种，核心公式如下公式所示：
...
![Bang-Bang 控制：仅按误差符号在 ±τ_max 间切换，关节穿过目标点形成极限环](./figs/bang-bang-control.webp)
...
![极限环时域图：关节在 q_d 两侧以幅度 Δq 周期摆动](./figs/bang-bang-limit-cycle.webp)

## 1.4 PID 控制

PID 控制（Proportional-Integral-Derivative Control）是工业控制领域应用最广泛、最经典的一种反馈控制算法，其核心公式如下公式所示：
...
![PID 三项分解：现在(P)、过去(I)、未来(D) 对阶跃响应的影响](./figs/pid-three-terms.webp)
...
![积分饱和：障碍解除瞬间累积的 I 项集中释放，关节剧烈过冲](./figs/integral-windup.webp)
...
![PD 等效弹簧-阻尼器：K_p 决定回拉刚度，K_d 决定阻尼，二者共同塑造阶跃响应](./figs/pd-spring-damper.webp)
...
我们可以用理论分析来指导调参，为此先用最简化的单关节模型：把关节看作一个绕固定轴转动的刚体，转动惯量为 $I$，忽略重力和摩擦。它的动力学方程如下公式所示：
...
其中 $\tau$ 是作用在关节上的总力矩——简化之后只剩 PD 控制器的输出。把 PD 律 $\tau = K_p\,(q_d - q) + K_d\,(\dot q_d - \dot q)$ 代入（取目标静止 $\dot q_d = 0$），就得到 PD 控制下的关节动力学，如下公式所示：
...
![实验 1：固定 Kp=40，只改变 Kd 时的单摆响应曲线](./figs/pd-kd-sweep.webp)
...
![实验 2：3 秒后加入恒定外扰，P 和 PD 都会留下稳态误差](./figs/pd-disturbance-error.webp)
