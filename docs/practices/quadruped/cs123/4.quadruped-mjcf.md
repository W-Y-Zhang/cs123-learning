...
![Pupper v3 总体结构：1 个机身 + 4 条结构对称的腿，单腿 3 关节（HAA / HFE / KFE）](./figs/quadruped-overall-structure.webp)
...
![pos 决定安装位置，quat 旋转局部坐标系，axis 定义局部转轴。](./figs/mjcf-quat-axis.svg)
...
![MuJoCo viewer 中的 Pupper v3 fixed 模型](./figs/lab4_viewer_home_pose.webp)
...
![固定基座与浮动基座：前者将机身固定在世界坐标系中，后者通过 freejoint 获得 6 个被动自由度，并在重力作用下运动。](./figs/fixed-floating-comparison.webp)
...
![Pupper 自由落地并保持 home 站姿](./figs/pupper-stand.gif)
...
![original、long-leg 和 heavy 三种 Pupper 形态及其对应站姿](./figs/lab4_pupper_zoo.webp)

三种模型在各自站姿和控制参数下均能稳定站立，但平衡高度不同：

![三种 Pupper 形态的机身高度曲线](./figs/lab4_stand_z_vs_t.webp)

PD 热力图用于检查不同参数下的高度波动，颜色表示最后 1 秒 `base z` 的标准差：

![三种 Pupper 形态的站立稳定性参数扫描](./figs/lab4_pd_heatmap.webp)
