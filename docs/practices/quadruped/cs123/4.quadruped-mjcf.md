...
[pupper_v3_fixed.xml](https://github.com/datawhalechina/dive-into-embodied-ai/blob/master/codes/practices/quadruped/cs123/assets/mjcfs/pupper_v3_fixed.xml) 在 `<mujoco>` 根节点下并列挂着几个顶层模块：
...
- [pupper_v3_fixed.xml](https://github.com/datawhalechina/dive-into-embodied-ai/blob/master/codes/practices/quadruped/cs123/assets/mjcfs/pupper_v3_fixed.xml)：把 `base_link` 焊在世界里，只留 12 个关节，方便先把腿单独调通。
- [pupper_v3.xml](https://github.com/datawhalechina/dive-into-embodied-ai/blob/master/codes/practices/quadruped/cs123/assets/mjcfs/pupper_v3.xml)：在 `base_link` 上加一行 `<freejoint name="world_to_body"/>`，机身有 6 个被动 DoF，会受重力作用。
...
参考[codes/practices/quadruped/cs123/README.md](https://github.com/datawhalechina/dive-into-embodied-ai/blob/master/codes/practices/quadruped/cs123/README.md) 用 [uv](https://docs.astral.sh/uv/) 安装环境：
...
fixed.xml 已经搭好，用 Python 加载它并打开 viewer，核心流程就三步：加载模型、应用 home keyframe、开渲染循环。配套代码里的 [run_view_pupper_fixed.py](https://github.com/datawhalechina/dive-into-embodied-ai/blob/master/codes/practices/quadruped/cs123/4.quadruped-mjcf/run_view_pupper_fixed.py) 也是这三步，只是外面多包了一层路径处理、相机设置和报错提示。
...
model = mujoco.MjModel.from_xml_path("../assets/mjcfs/pupper_v3_fixed.xml")
...
所以这节页面里的代码不是另一份实现，而是把 `run_view_pupper_fixed.py` 拆开讲：先解释最小 API，再解释完整脚本为什么要多写这些包装。
...
# 或 python run_view_pupper_fixed.py  # Linux / Windows
```

正常情况下会先在终端打印一行模型规模，然后弹出 viewer 窗口：

```
Loaded: .../assets/mjcfs/pupper_v3_fixed.xml
...
仓库里已经放了一份改好的 [pupper_v3.xml](https://github.com/datawhalechina/dive-into-embodied-ai/blob/master/codes/practices/quadruped/cs123/assets/mjcfs/pupper_v3.xml)，可以直接对照。为了方便动手，[pupper_v3_fixed.xml](https://github.com/datawhalechina/dive-into-embodied-ai/blob/master/codes/practices/quadruped/cs123/assets/mjcfs/pupper_v3_fixed.xml) 里也预留了 Lab 4.4 的注释开关：想临时把 fixed 改成 floating，只需要改两处。
...
配套代码里给了一份 [run_view_pupper.py](https://github.com/datawhalechina/dive-into-embodied-ai/blob/master/codes/practices/quadruped/cs123/4.quadruped-mjcf/run_view_pupper.py)：主循环用 `mj_step` 推进物理（不是 [§4.3](/docs/practices/quadruped/cs123/quadruped-mjcf#43-在-viewer-中查看模型) 那个 fixed 脚本里的 `mj_forward`），所以会演化重力和接触，能看到机身从 `z=0.28 m` 落下、四脚触地、再被位置伺服把关节拉回目标角的全过程——这就是 [§4.5](/docs/practices/quadruped/cs123/quadruped-mjcf#45-让-pupper-站起来)"站住"的物理基础。

```bash
cd codes/practices/quadruped/cs123/4.quadruped-mjcf
mjpython run_view_pupper.py    # macOS
# 或 python run_view_pupper.py # Linux / Windows
```
...
> **macOS 上不要走 `mjpython -m mujoco.viewer --mjcf=...`**：mjpython 启动时为了拿 GUI 主线程已经 import 过一次 `mujoco.viewer`，再用 `-m` 通过 runpy 跑第二次会撞出 `RuntimeError: Caught an unknown exception!`（错在 `_Simulate(...)`，与 XML 内容无关）。脚本入口 `mjpython script.py` 绕开这个问题。Linux / Windows 不依赖 `mjpython`，`python -m mujoco.viewer --mjcf=../assets/mjcfs/pupper_v3.xml` 仍然可用。
...
[§4.4](/docs/practices/quadruped/cs123/quadruped-mjcf#44-加-freejoint-改-floating) 的 [run_view_pupper.py](https://github.com/datawhalechina/dive-into-embodied-ai/blob/master/codes/practices/quadruped/cs123/4.quadruped-mjcf/run_view_pupper.py) 已经把这两件事都跑通了——Pupper 落地后会被 PD 拉到 home keyframe 的 12 个目标角（全 0），稳稳站在地上，base z 稳在 0.18 m 附近。看起来反直觉的是：12 个关节角都是 0，怎么不是"四脚直伸贴地"？这要回到 [§4.2.2](/docs/practices/quadruped/cs123/quadruped-mjcf#422-搭运动学树) 那条 quat 规则——Pupper 的 mesh 和各级 body 的 `quat` 都是按"关节角 0 = 膝盖屈、机身托起"装配的，全 0 ctrl 落在屈膝站姿上，是仓库里写死的"home 即站姿"约定。

那 §4.5 在 §4.4 基础上还要做什么？两件事：每帧**显式**把伺服目标写一遍 `data.ctrl`（保持目标不被任何外部代码意外改写——[§5](/docs/practices/quadruped/cs123/gait-control) 真正写步态时就会在循环里改它）、记录 `data.qpos[2]` 时间序列作为站住的客观判据。配套代码 [run_stand_pupper.py](https://github.com/datawhalechina/dive-into-embodied-ai/blob/master/codes/practices/quadruped/cs123/4.quadruped-mjcf/run_stand_pupper.py)，相对 floating 脚本真正不同的就这几行：
...
# 或 python run_stand_pupper.py # Linux / Windows
```
...
肉眼看视频不够客观。判 pass 的标准是后期 base z 的稳定性，`run_stand_pupper.py` 在 viewer 关闭时会自动打印一行：
...
```text
codes/practices/quadruped/cs123/4.quadruped-mjcf/run_gain_sweep.py
```

它每次都从同一份 `pupper_v3.xml` 加载模型，然后在内存里改 actuator 参数，不会写回 XML 文件。先直接跑：
...
也就是说，你想加一组新实验，不需要改 XML，只要打开 `run_gain_sweep.py`，在 `CASES` 里多放一个 `GainCase("name", kp, kd, "...")`，重新运行脚本即可。

如果你想单独看某一组的实时效果，用同一个脚本打开 viewer：

```bash
# macOS
mjpython run_gain_sweep.py --viewer soft
mjpython run_gain_sweep.py --viewer default
mjpython run_gain_sweep.py --viewer stiff
mjpython run_gain_sweep.py --viewer underD

# Linux / Windows
python run_gain_sweep.py --viewer stiff
```

viewer 模式也会在第 2 秒施加同样的下压扰动。只想看不加扰动的自由落体站立过程，就加 `--no-push`：

```bash
python run_gain_sweep.py --viewer default --no-push
```

如果你的机器没有可用的 MuJoCo offscreen renderer，批量脚本可能生成不了 PNG / GIF。这时先跑纯数值版：

```bash
python run_gain_sweep.py --no-render
```
