...
- [pupper_v3.xml](https://github.com/datawhalechina/dive-into-embodied-ai/blob/master/codes/practices/quadruped/cs123/4.quadruped-mjcf/models/pupper_v3.xml)：在 `base_link` 上加一行 `<freejoint name="world_to_body"/>`，机身有 6 个被动 DoF，会受重力作用。
...
仓库里已经放了一份改好的 [pupper_v3.xml](https://github.com/datawhalechina/dive-into-embodied-ai/blob/master/codes/practices/quadruped/cs123/4.quadruped-mjcf/models/pupper_v3.xml)，可以直接对照。为了方便动手，[pupper_v3_fixed.xml](https://github.com/datawhalechina/dive-into-embodied-ai/blob/master/codes/practices/quadruped/cs123/4.quadruped-mjcf/models/pupper_v3_fixed.xml) 里也预留了 Lab 4.4 的注释开关：想临时把 fixed 改成 floating，只需要改两处。
...
> **macOS 上不要走 `mjpython -m mujoco.viewer --mjcf=...`**：mjpython 启动时为了拿 GUI 主线程已经 import 过一次 `mujoco.viewer`，再用 `-m` 通过 runpy 跑第二次会撞出 `RuntimeError: Caught an unknown exception!`（错在 `_Simulate(...)`，与 XML 内容无关）。脚本入口 `mjpython script.py` 绕开这个问题。Linux / Windows 不依赖 `mjpython`，`python -m mujoco.viewer --mjcf=models/pupper_v3.xml` 仍然可用。
